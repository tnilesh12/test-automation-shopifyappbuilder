...
class ApiRepositoryWithoutBaseUrl {
  // final Dio client;
  // ApiRepository(this.client);
  static Dio client = Dio();

  static Future<ApiResponse> getAPI(String apiName, {String token = ""}) async {
  ...
  static Future<dynamic> deleteAPI(String apiName, {String token = "", Object? data}) async {
    if (await Utils.isInternetConnected()) {
      try {
        String callingUrl = apiName;
        debugPrint("-------------------------");
        debugPrint("url:-------------- ${callingUrl}");
        debugPrint("token:------------ ${token}");
        if (token.isNotEmpty) {
  ...
  static Future<dynamic> postAPI(String apiName, var formData, {String token = ""}) async {
    if (await Utils.isInternetConnected()) {
      try {
        String callingUrl = apiName;
  ...
  static Future<dynamic> putAPI(String apiName, var formData, {String token = ""}) async {
    if (await Utils.isInternetConnected()) {
      try {
        String callingUrl = apiName;

...
class JsonHelper {
  ...
  static String chooseRightOrderOnCurrencySymbol(
    ...
    bool isPrefix =
        globals.Settings.containsKey(SettingsEnum.Currency_Format.name)
            ? globals.Settings[SettingsEnum.Currency_Format.name].otherData
                        .position ==
                    "prefix"
                ? true
                : false
        ...
        globals.Settings.containsKey(SettingsEnum.Currency_Format.name)
            ? globals.Settings[SettingsEnum.Currency_Format.name].otherData
                ...
                ? globals.Settings[SettingsEnum.Currency_Format.name].otherData
                    .symbol
                    .trim()
                : '${_simpleCurrencySymbols[currencyCode]}'
            : '${_simpleCurrencySymbols[currencyCode]}';

    // Custom currency formatting
    NumberFormat customCurrencyFormat = NumberFormat.currency(
      locale: 'en_US',
      symbol: '', // Set an empty string to avoid currency code
      decimalDigits: 2,
    );

    String formattedAmount =
        customCurrencyFormat.format(amountFromJson(amount));

    if (isPrefix) {
      formattedAmount =
          '$symbol ' + formattedAmount; // Prefix the currency symbol
    } else {
      formattedAmount =
          formattedAmount + ' $symbol'; // Postfix the currency symbol
    }

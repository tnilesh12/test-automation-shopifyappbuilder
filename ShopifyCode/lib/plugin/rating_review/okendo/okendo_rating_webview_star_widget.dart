...
class OkendoRatingWebViewStarWidget extends StatefulWidget {
  String productId;

  OkendoRatingWebViewStarWidget(this.productId) {}

  @override
  State<OkendoRatingWebViewStarWidget> createState() =>
      _OkendoRatingWebViewStarWidgetState();
}

class _OkendoRatingWebViewStarWidgetState
    extends State<OkendoRatingWebViewStarWidget> {
  ...
  _loadContent(String productId) {
    try {
      print("----------${Uri.parse(ApiConst.okendoReviewSecond.replaceAll("{product_id}", productId).replaceAll("{subscriber_id}",globals.plugins[PluginsEnum.OKENDO_REVIEW.name].secrets.subscriberId))}------------");
